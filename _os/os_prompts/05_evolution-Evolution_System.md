...
Before ANY evolution move — FAST, DEEP, RESEARCH, or INBOX — the agent MUST read the entity runtime `evolution_objectives` and keep them in scope for the whole run. Pillars + evolution_objectives each have TWO buckets — `validated` (created directly by the user) and `suggestions` (generated by the agent) — and EVERY entry carries its own `status: active | not`. **The agent reads ONLY the ACTIVE entries, regardless of which bucket they live in.** "Activate" (the user's preferred verb) = flipping an entry's `status` to active (and registering its name in the section's `actives` list) — it is NOT a move between buckets; the entry stays where it was. The bucket is provenance; `status: active` is the usability switch that makes an entry usable in workflows. If `evolution_objectives` has no active entry, seed/activate one first before running evolution.
