...
<entity>-inbox/.gateway/        agent-curated:
...
2. Daemon detects them, stamps structure in `*-inbox.yaml -> raw`, flags fill_queue.inbox.
3. Agent describes each raw item (description/contains/when_to_use).
4. Agent delivers (copies, never moves) relevant items into .gateway/<Pillar>/<functional_group>/, recording extracted_concern + source_raw_item. Raw stays untouched.
