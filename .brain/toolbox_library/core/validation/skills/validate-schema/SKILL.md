...
1. **Locate and read the schema.** Find the authoritative schema definition:
   - For workspace YAML files → check `meta.router.yaml` `expected_schema` or `expected_file_schemas`
...
- Validation failures on `CONTROLER.yaml` or router files are critical — they corrupt agent routing.
