...
| **5. Goal Mgmt** | `task{}` + `context_scan[]` | `goal{}` | Goal exists in `.mission_board/` AND (status != done OR is persistent) | If goal is `done ✅` (non-persistent), ⚠️ SOFT PASS (mark task complete, pick next). Else ❌ RETRY creation. |
| **6. Context Deep** | `goal{}` | `context_deep[]` | Goal folder exists in `.mission_board/[SESSION_ID]/[GOAL_ID]/` | ✅ PROCEED even if empty |
...
| **10. Sync** | `result{}` | Sync actions | `CONTROLER.yaml` write succeeded. If goal is `persistent ♾️`, do NOT mark done, log cycle. **Update `sessions.active[].last_action`.** | ❌ RETRY. If fail → log to `.mission_board/[SESSION]/[GOAL]/scratch/` |
...
- **Before Execution:** The agent MUST read and execute `.brain/meta.router/.sync_engine/sync_mission_board.md`, `sync_toolbox.md`, and `sync_pipelines.md` to ensure the physical files and maps are healthy.

| Scenario | work_mode | Action |
|----------|-----------|--------|
| Active session exists | AUTO | **Auto-resume**: Load session context from `.mission_board/`, continue |
| Active session exists | STRICT/COLLAB | **Ask**: "Continue SES-XXX or start new?" |
| No sessions exist | Any | **Start new**: Generate `SES-[NNN]`, register in `CONTROLER.yaml` and `.mission_board/` |
| Stale session (> 24h idle) | Any | Mark `status: abandoned`, start new |

### Session Lifecycle & End-of-Session Sync
1. **START** → Generate `SES-[NNN]` (incrementing). Create `.mission_board/[SES-NNN]/` and its `.yaml`.
...
- **Details & Artifacts** → Must be stored in the specific goal directory: `.brain/.mission_board/[SESSION_ID]/[GOAL_ID]/`
