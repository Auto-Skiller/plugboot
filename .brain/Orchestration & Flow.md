...
| **4. Context Scan** | `task{}` + context catalogs | `context_scan[]` | — | ✅ PROCEED even if empty (log warning) |
...
- **At boot:** Refresh ALL registries (Navigator → Cataloger full chain)
- **After execution (Step 10):** Mark affected scope registries as `status: stale`
- **Next boot:** Stale registries get refreshed
...
3. The agent appends these learnings to `.scope/.core/.context/knowledge.md` or `.scope/.core/.context/workflows.md`.
