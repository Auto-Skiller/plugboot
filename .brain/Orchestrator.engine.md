...
| **Cataloger** | Hybrid | Compares scan to existing registry, flags changes, generates/writes descriptions. |
...
| **Index Refresh**| Navigator → Cataloger | A scope was modified, registry needs updating without routing |
...
1. **Agent Orchestration**: The Agent acts as the Orchestrator. The Agent decides which chain to run based on the `status` field inside the `.registry` files.
2. **Never Route on Stale**: If a registry is `status: stale`, the Router CANNOT use it. It must trigger Navigator → Cataloger first.
