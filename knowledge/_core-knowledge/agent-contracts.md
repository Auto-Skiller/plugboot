...
Completion markers and handoff schemas for all System agents. Workflows use these markers to detect agent completion and route accordingly.
...
|-------|------|--------------------|
| planner | Plan creation | `## PLANNING COMPLETE` |
| executor | Plan execution | `## PLAN COMPLETE`, `## CHECKPOINT REACHED` |
| phase-researcher | Phase-scoped research | `## RESEARCH COMPLETE`, `## RESEARCH BLOCKED` |
| project-researcher | Project-wide research | `## RESEARCH COMPLETE`, `## RESEARCH BLOCKED` |
| plan-checker | Plan validation | `## VERIFICATION PASSED`, `## ISSUES FOUND` |
| research-synthesizer | Multi-research synthesis | `## SYNTHESIS COMPLETE`, `## SYNTHESIS BLOCKED` |
| debugger | Debug investigation | `## DEBUG COMPLETE`, `## ROOT CAUSE FOUND`, `## CHECKPOINT REACHED` |
| roadmapper | Roadmap creation/revision | `## ROADMAP CREATED`, `## ROADMAP REVISED`, `## ROADMAP BLOCKED` |
| ui-auditor | UI review | `## UI REVIEW COMPLETE` |
| ui-checker | UI validation | `## ISSUES FOUND` |
| ui-researcher | UI spec creation | `## UI-SPEC COMPLETE`, `## UI-SPEC BLOCKED` |
| verifier | Post-execution verification | `## Verification Complete` (title case) |
| integration-checker | Cross-phase integration check | `## Integration Check Complete` (title case) |
| nyquist-auditor | Sampling audit | `## PARTIAL`, `## ESCALATE` (non-standard) |
| security-auditor | Security audit | `## OPEN_THREATS`, `## ESCALATE` (non-standard) |
| codebase-mapper | Codebase analysis | No marker (writes docs directly) |
| assumptions-analyzer | Assumption extraction | No marker (returns `## Assumptions` sections) |
| doc-verifier | Doc validation | No marker (writes JSON to `.planning/tmp/`) |
| doc-writer | Doc generation | No marker (writes docs directly) |
| advisor-researcher | Advisory research | No marker (utility agent) |
| user-profiler | User profiling | No marker (returns JSON in analysis tags) |
| intel-updater | Codebase intelligence analysis | `## INTEL UPDATE COMPLETE`, `## INTEL UPDATE FAILED` |

## Marker Rules

1. **ALL-CAPS markers** (e.g., `## PLANNING COMPLETE`) are the standard convention
2. **Title-case markers** (e.g., `## Verification Complete`) exist in verifier and integration-checker -- these are intentional as-is, not bugs
