<overview>
Git integration for System framework.
</overview>
...
If NO_GIT: Run `git init` silently. System projects always get their own repo.
</git_check>
...
```bash
sdk query commit "docs: initialize [project-name] ([N] phases)" .planning/
```
...
```bash
sdk query commit "docs({phase}-{plan}): complete [plan-name] plan" .planning/phases/XX-name/{phase}-{plan}-PLAN.md .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
sdk query commit "wip: [phase-name] paused at task [X]/[Y]" .planning/
```
...
For workspaces with separate git repos (e.g., `backend/`, `frontend/`, `shared/`), System routes commits to each repo independently.
...
3. **Independent commits:** Each sub-repo receives its own atomic commit via `tools.cjs commit-to-subrepo`. File paths are made relative to the sub-repo root before staging.
...
```bash
sdk query commit-to-subrepo "feat(02-01): add user API" \
