...
"git": {
  ...
  "base_branch": null,
  "phase_branch_template": "System/phase-{phase}-{slug}",
  "milestone_branch_template": "System/{milestone}-{slug}",
...
| `git.phase_branch_template` | `"System/phase-{phase}-{slug}"` | Branch template for phase strategy |
| `git.milestone_branch_template` | `"System/{milestone}-{slug}"` | Branch template for milestone strategy |
...
| `manager.flags.discuss` | `""` | Flags passed to `/discuss-phase` when dispatched from manager (e.g. `"--auto --analyze"`) |
...
**Using `sdk query` (preferred):**

```bash
# Commit with automatic commit_docs + gitignore checks:
sdk query commit "docs: update state" .planning/STATE.md

# Load config via state load (returns JSON):
INIT=$(sdk query state.load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
# commit_docs is available in the JSON output

# Or use init commands which include commit_docs:
INIT=$(sdk query init.execute-phase "1")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
...
**Note:** Most System operations use direct file reads or explicit paths, which work regardless of gitignore status.
...
- Branch name from `phase_branch_template` (e.g., `System/phase-03-authentication`)
...
- Branch name from `milestone_branch_template` (e.g., `System/v1.0-mvp`)
...
| `review.models.<cli>` | string\|null | `null` | Any model ID string | Per-CLI model override for /review (e.g., `review.models.gemini`). Falls back to CLI default when null. |
...
| `workflow.ai_integration_phase` | boolean | `true` | `true`, `false` | Run /ai-integration-phase before planning AI system phases |
...
| `workflow.security_enforcement` | boolean | `true` | `true`, `false` | Enable threat-model-anchored security verification via `/secure-phase`. When `false`, security checks are skipped entirely |
...
| `git.phase_branch_template` | string | `"System/phase-{phase}-{slug}"` | Template with `{phase}`, `{slug}` | Branch naming template for `phase` strategy |
| `git.milestone_branch_template` | string | `"System/{milestone}-{slug}"` | Template with `{milestone}`, `{slug}` | Branch naming template for `milestone` strategy |
...
| `features.global_learnings` | boolean | `false` | `true`, `false` | Enable injection of global learnings from `~/.System/learnings/` into agent prompts |
...
Set via `intel.*` namespace (e.g., `"intel": { "enabled": true }`). Controls the queryable codebase intelligence system consumed by `/intel`.

| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `intel.enabled` | boolean | `false` | `true`, `false` | Enable queryable codebase intelligence system. When `true`, `/intel` commands build and query a JSON index in `.planning/intel/`. |
...
| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `manager.flags.discuss` | string | `""` | Any CLI flags string | Flags passed to `/discuss-phase` from manager (e.g., `"--auto --analyze"`) |
...
5. **Search API keys and flags** -- `brave_search`, `firecrawl`, and `exa_search` are auto-set to `true` during project creation if the corresponding API key is detected (environment variable or `~/.System/<name>_api_key` file). Setting them to `true` without the API key has no effect.
...
8. **`sub_repos` auto-sync** -- On every config load, System scans for child directories with `.git` and updates the `sub_repos` array if the filesystem has changed. `multiRepo: true` is automatically migrated to a detected `sub_repos` array.
