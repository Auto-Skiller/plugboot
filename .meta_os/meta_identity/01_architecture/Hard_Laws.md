...
**Never guess file paths.** All paths must come from a router (`CONTROLER.yaml`, a `.meta_os/meta_db/` file) or a physical directory listing. Hallucinating a path is a critical protocol violation.

## 3. Localized Master Environments
We do NOT install global tools on the host OS. Every runtime must be self-contained.
*   **Python:** Use the master environment at `.meta/venv/`.
...
*   **The Rule:** Always invoke tools via the cross-platform launcher at `.meta/venv/`.
*   **Windows:** `.\.meta\venv\meta_run.ps1 -m script_name`
*   **Linux/macOS:** `./.meta/venv/meta_run.sh -m script_name`
