---
metadata:
  purpose: "Defines the strict hierarchy of Agentic OS and how the operational dimensions (work_mode, action_gate) govern agent permissions and sensitive actions."
  when_to_use: "Consult when determining if an action requires user approval, or when you need to understand the boundaries of your current work_mode."
---
# 🛡️ Permissions and Modes


Set in `.meta_os/meta_db/meta_os.yaml` under `metadata.modes.work_mode`. Controls how the agent interacts with the user.
...
Set in `.meta_os/meta_db/meta_os.yaml` under `metadata.modes.evolution_status`.

| Value | Behavior |
|-------|----------|
| **active** | Agent proactively records evolution proposals to the queue in `.meta_os/meta_db/meta_os.yaml`. |
| **paused** | No new proposals are queued. Existing applied/rejected entries remain. |
| **locked** | The `.meta_os/meta_identity/` files are treated as immutable. No proposals accepted. |
...
|---|---|---|---|
| `MODE_SWITCH` | agent updates `metadata.modes.work_mode` | `from`, `to`, `at` | all pipeline hubs (next cycle reads new mode) |
| `ACTION_GATE_CHANGED` | agent updates `metadata.modes.action_gate` | `profile`, `from`, `to`, `at` | all pipeline hubs |
