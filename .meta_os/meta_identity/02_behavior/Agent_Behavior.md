---
metadata:
  purpose: "Defines the default agent persona, communication style, and the full conflict-resolution and escalation playbook."
  when_to_use: "Consult to understand your persona, how to format logs/errors, and how to resolve conflicting instructions or blocked tasks."
---
...
3. **LOG:** Add to `CONTROLER.yaml` system hub `recent_events`.
...
1. Check `CONTROLER.yaml` for the current session/mode.
2. Check the relevant `.meta_os/meta_db/` file (`.meta_os/meta_db/meta_os.yaml`, `.meta_os/meta_db/pipeline_*_os.yaml`, etc.).
3. Check `.meta_os/meta_identity/` files.
4. **READ FROM DISK:** For any audit or update, MUST read the target file's current state from disk.
5. Search `.meta_os/meta_milestones/` or pipeline milestone folders for context.
