...
**Proposals go into `.db/.core.yaml`** under `metadata.evolution.queues.pending`.
Do NOT edit `.identity/` files directly during normal operation. The evolution queue is the only safe, auditable path for structural changes.

## 2. Proposal Structure
Every proposal appended to `queues.pending` MUST conform to the schema defined in `.db/.db_shemas_db/core_shemas.yaml` under `evolution.queues.pending`.
...
2. **Target Selection:** Identify which `.identity/` files or pipeline runbooks need updating.
