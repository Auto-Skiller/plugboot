...
- **Implementation:** `_os/engine/_shared/sync_lock.py` (`acquire()` / `release()` / `inspect()`).
...
All YAML writes go through `_os/engine/_shared/atomic_io.py` — write to `<file>.<pid>.tmp`, then `os.replace` over the target. A crashed or killed daemon can never leave a half-written file. Tmp orphans are swept on next daemon start.

- **Files protected:** `CONTROLER.yaml`, all `.db/*.yaml`, all `.db/toolboxes_db/**/*.yaml`, all milestone session files.
...
Multiple agents may queue evolution proposals during a long autonomous run. The contract for appending to `metadata.evolution.queues.pending` in `.db/.core.yaml`:

1. Acquire the daemon lock before append.
2. Read `.db/.core.yaml` with `load_yaml`.
3. Mutate the `pending` list locally.
4. Write back via `atomic_write_yaml`.
5. Release the lock.

A single helper `_os/engine/_shared/state_helpers.py` → `append_pending_evolution(proposal)` encapsulates this. Hand-written append code is forbidden.
