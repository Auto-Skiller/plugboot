...
### 1. Domain Routing
All toolboxes live under `_toolboxes/`. The `.db/.toolboxes.yaml` file is the index.

| Domain | Physical Path | DB Index Path |
|--------|--------------|---------------|
| **Core** | `_toolboxes/.core_toolboxes/` | `.db/toolboxes_db/.core_toolboxes_db/` |
| **Business** | `_toolboxes/business_toolboxes/` | `.db/toolboxes_db/business_toolboxes_db/` |
| **Engineering** | `_toolboxes/engineering_toolboxes/` | `.db/toolboxes_db/engineering_toolboxes_db/` |
| **Life** | `_toolboxes/life_toolboxes/` | `.db/toolboxes_db/life_toolboxes_db/` |
| **Studio** | `_toolboxes/studio_toolboxes/` | `.db/toolboxes_db/studio_toolboxes_db/` |
...
- **Agents:** Sit under `_toolboxes/<domain>/<toolbox>/agents/<AGENT_NAME>/AGENT.md`
- **Skills:** Sit under `_toolboxes/<domain>/<toolbox>/skills/<SKILL_NAME>/SKILL.md`
...
- Cross-toolbox relationships are tracked in `.db/.toolboxes.yaml` under `dependency_graph`. Add new edges when building agents that rely on multiple toolboxes.

### 4. Extending Toolboxes
To add a new capability:
1. Create the `skills/` files (`SKILL.md` + script) inside the appropriate `_toolboxes/` subfolder. Ensure they include rich YAML frontmatter.
...
- **Read First:** Read the projects DB (`.db/projects.yaml`) before touching any file under `projects/` to understand the project's stack and entry point.
- **Registration:** When creating a new project, add its entry to `.db/projects.yaml` under `metadata.projects`.
...
4. **Context Scan**: Read relevant `.db/` files and `.identity/` files.
