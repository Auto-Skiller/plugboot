...
Set in `.db/.core.yaml` under `metadata.modes.work_mode`. Controls how the agent interacts with the user.
...
Set in `.db/.core.yaml` under `metadata.modes.evolution_status`.

| Value | Behavior |
|-------|----------|
| **active** | Agent proactively records evolution proposals to the queue in `.db/.core.yaml`. |
| **paused** | No new proposals are queued. Existing applied/rejected entries remain. |
| **locked** | The `.identity/` files are treated as immutable. No proposals accepted. |
