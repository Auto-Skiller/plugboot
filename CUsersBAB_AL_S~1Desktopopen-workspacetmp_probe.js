...
function renderCoreModes(modes, coreBlock) {
  ...
  // ── System Hub Events ──
  const sysEvents = coreBlock?.system?.hub?.recent_events || [];
  ...
  const errors = coreBlock?.system?.system_errors || [];
  ...
  const evoEvents = coreBlock?.evolution?.hub?.recent_events || [];
  ...
  const coreMs   = coreBlock?.milestones || {};
