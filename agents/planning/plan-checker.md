...
```bash
INIT=$(sdk query init.phase-op "${PHASE_ARG}")
...
```bash
node ./node_modules/@build/sdk/dist/cli.js query phase.list-plans "$phase_number"
# Research / brief artifacts (deterministic listing)
node ./node_modules/@build/sdk/dist/cli.js query phase.list-artifacts "$phase_number" --type research
node ./node_modules/@build/sdk/dist/cli.js query roadmap.get-phase "$phase_number"
node ./node_modules/@build/sdk/dist/cli.js query phase.list-artifacts "$phase_number" --type summary
```
...
Use `sdk query` to validate plan structure:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(sdk query verify.plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
Extract must_haves from each plan using `sdk query`:

```bash
MUST_HAVES=$(sdk query frontmatter.get "$PLAN_PATH" must_haves)
```
...
```bash
PLAN_STRUCTURE=$(sdk query verify.plan-structure "$PLAN_PATH")
```
...
```bash
node ./node_modules/@build/sdk/dist/cli.js query plan.task-structure "$PLAN_PATH"
```
...
```bash
node ./node_modules/@build/sdk/dist/cli.js query plan.task-structure "$PHASE_DIR/$PHASE-01-PLAN.md"
node ./node_modules/@build/sdk/dist/cli.js query frontmatter.get "$PHASE_DIR/$PHASE-01-PLAN.md" files_modified
```
