...
credentials:
  description: Cascading decision tree, threshold definitions, 5 boundary signals,
    cluster-first rule, and validation checklist
  when_to_use: Read when deciding whether to cascade a signal into an existing focus/product/feature
  contains: decision_tree, thresholds, cluster_rule
---
...
└── NO → continue:
    │
    ├── 1. Check for Matching Focus (in central index.yaml rollups)
    │   │
...
- [ ] **`system-board.yaml` phase update prepared**: `state.current_phase` will reflect Phase 2 cascade completion atomically with the writes.
...
5. **Trigger sync**: Run `.infra/backend/engine.py` so central `index.yaml` rollups reflect new totals.
