...
| `HUSTLE_SESSION_OPENED` | New `HUSTLE-[Market]-[ID]` session opened (H-LAW-001) | `hustle_id`, `market`, `product_ref`, `roi_projection_ref`, `at` | central `index.yaml` rollups, milestones |
...
| Event | Emitted by | Payload | Consumer |
|---|---|---|---|
| `STATE_SYNC` | `system-board.yaml` mirrors .system.board pre-cycle | `mirrored_fields[]`, `at` | `system-board.yaml` |
| `LEDGER_ROLLUP_REASSEMBLED` | Auto-aggregator rebuilt central `index.yaml` rollups | `focuses_aggregated[]`, `at` | router |
| `RUNTIME_ROLLUP_REASSEMBLED` | Auto-aggregator rebuilt central `index.yaml` rollups | `at` | router |
| `RUNBOOK_READINESS_LOGGED` | H-LAW-010 acknowledgement (when enforced) | `runbook_file`, `content_hash`, `at` | `system-board.yaml.runbook_readiness` |
...
5. **Audit-Pass emissions are observation-only.** Audit events do not cause writes outside `system-board.yaml` and the remediation message — they describe state, never change it.
