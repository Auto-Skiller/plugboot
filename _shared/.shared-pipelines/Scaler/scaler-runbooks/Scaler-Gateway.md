...
> **PREVENTION RULE**: Before creating any card, check `system-board.yaml → gateway_metrics.pending_approvals_count`. If > 0, address or acknowledge pending cards first before creating new ones (unless unrelated scope).
...
1. Read `system-board.yaml` → confirm current phase and gateway metrics.
...
4. Update `system-board.yaml → state.current_phase` to the appropriate phase.
...
- **EXTERNAL**: Update the EXISTING entry in `[Pillar].sources_ledger.yaml.state.tracked_discoveries[]` (using its content hash). Append the new card's ID to `proposal_ids[]` and set `integration_status: PENDING`. The auto-sync rolls this into the central `index.yaml` rollups.
...
- Update `system-board.yaml → gateway_metrics.pending_approvals_count` (increment by 1 if PLANNING mode).
- Update `system-board.yaml → gateway_metrics.last_gateway_action`.
...
After card + ledger are both written, the Scaler checks the `action_gate` map in `system-board.yaml` for the card's `integration_type`:
...
- Post a review request in `index.yamlcommunication.scaler_review_queue`:
  ```yaml
  - id: [card_id]
    ...
    status: PENDING
  ```
- Update `system-board.yaml → active_triage[]` with the card.
...
- **`APPROVED`**: Set `user_decision: APPROVED` in the card. Update review queue entry `status: APPROVED`. Atomically update `system-board.yaml` to decrement `gateway_metrics.pending_approvals_count` by 1 and increment `gateway_metrics.integration_queue_count` by 1. Proceed to Step 5.
...
- [ ] `action_gate_at_creation` mirrors current .system.board value
- [ ] Ledger entry created in same operation
- [ ] `system-board.yaml` updated (phase, metrics, gateway_metrics)
...
> **PREVENTION — New Aspect Rule**: If analysis reveals a need for a new aspect/scope, NEVER create it directly. Post in `index.yamlsystem_status.scope_suggestions[]` and halt. Await explicit user approval before creating any folder structure for a new aspect. This applies in ALL modes (PLANNING and EXECUTION).
...
> This protocol closes the loop between Scaler discovery and `_shared/.shared-toolboxes/` population.
...
integration_strategy:
  ...
  toolbox_target:
    toolbox_path: "_shared/.shared-toolboxes/engineering_toolboxes/coding"
    toolbox_yaml: "index.yamltoolboxes.rollups/engineering_toolboxes.rollups/coding.yaml"
...
1. Run `meta_sync.py` to remap `toolboxes.board.yaml`.
...
4. New cross-toolbox edges are added via UPGRADE cards under the
   `dependency_graph.edges` block of `system-board.yaml`.
...
| Operation | Card file | `[Pillar].sources_ledger.yaml` | `[Pillar].proposals_ledger.yaml` | `.scaler_mixed_inbox.ledger.yaml` | `system-board.yaml` | `system-board.yaml` | `system-board.yaml` (via sync) | Recovery |
|---|---|---|---|---|---|---|---|---|
...
| Operation | Card file | `[Pillar].proposals_ledger.yaml` | `[Pillar].sources_ledger.yaml` | `system-board.yaml` | `system-board.yaml` | `system-board.yaml` (via sync) | Recovery |
|---|---|---|---|---|---|---|---|
...
| Operation | `system-board.yaml` | `system-board.yaml` | `system-board.yaml` (via `.infra/backend/engine.py`) | `system-board.yaml` (auto) | `system-board.yaml` (auto) | Recovery |
|---|---|---|---|---|---|---|
