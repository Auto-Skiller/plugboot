...
credentials:
  description: Scaler discovery logic — classification, categorization, cluster intake
    protocol, functional groups
  when_to_use: Read when new external sources arrive — defines how to classify and
    route them
...
| **BUILD_NEW** | No matching target exists in the interrogated Pillar. | **Registry Compliance:** Must be registered in Correct Routers. | 1. Define Structure → 2. Create Files → 3. Register in `index.yaml` routers. |
...
4. **Human gate (last resort)** — if specificity AND recency tie AND no pending merge candidate exists, the Scaler MUST stop, post the tie in `system-board.yaml.communication_hubs.scaler_hub.scaler_review_queue` with `status: TIE_PENDING_RESOLUTION`, and resume only after the user picks a type. Never coin-flip; never default-bias to one type silently.
...
**Update order is mandatory**: pillar `sources_ledger` written first → master rollup at `system-board.yaml` re-aggregated by sync after.
