...
1. **Pillar Dominance**: The identity context (`_system/.system-meta/.system-os_prompts/`) rules always take precedence. You can adopt the discovery to become compatible with the current architecture.
2. **Deterministic Precedence**: The Master DB Index (`system-board.yaml`) is the final authority. If a proposal contradicts the Index without a plan for a safe migration, it must be rejected. We need an adopting plan for a safe migration.
...
1. `system-board.yaml` — session goal status, tracking, artifacts, last_sync, recent_events.
...
> **Known limitation (audit-acknowledged).** This rule is currently honor-system: no programmatic gate verifies the read. A future enhancement could store per-session runbook acknowledgements (timestamp + content-hash) in `system-board.yaml.runbook_readiness` and enforce pre-flight refusal on missing/stale acks. Carved out of the 2026-05-18 substrate audit (Cluster D — deferred). Until enforced, agents are bound by P-LAW-008 on trust.
...
This ensures that the global sync workers have updated their respective YAMLs and that `MASTER_INDEX.yaml` and `system-board.yaml` reflect the absolute truth of the on-disk state. Direct interpreter invocation (`.venv\Scripts\python.exe`) is prohibited per the cross-OS portability law.

**Verification Protocols:**
- **Pre-Flight Check**: Verify `system-board.yaml` exists and has a valid phase before initiating sync.
...
3. **Roll back any successful writes from the same operation.** Examples:
   - If the card file was written but `[Pillar].sources_ledger.yaml` failed to update → delete the card file. Revert `system-board.yaml.gateway_metrics` increments. Revert `scaler_review_queue` posting if any.
   - If both card and ledger succeeded but `system-board.yaml` failed → delete the card, remove the ledger entry, revert any review-queue posting. The trio is all-or-nothing.
...
4. **Log the failure** to `index.yamlcommunication_hubs.scaler_hub.messages` with:
...
- A discovery whose `unread_assets[]` count > 0 MUST be flagged in `system-board.yaml.state.deferred_assets_count`. The Scaler MAY still draft a Proposal Card from the readable subset, but the card MUST list the deferred assets in `scaler_notes`.
...
2. When executing an OS architectural audit, the scaler operates in INTERNAL mode. The identity documents (`_system/.system-meta/.system-os_prompts/`) serve as the runbook for structural changes.
...
5. **MANDATORY:** For EXTERNAL operations — consult the relevant `[Pillar].sources_ledger.yaml` (anti-duplication via content hash) BEFORE starting. Items in `.scaler_mixed_inbox/` also gate against `.scaler_mixed_inbox.ledger.yaml`. The auto-sync rolls all per-pillar split files into the central `index.yaml` rollups.

6. **MANDATORY:** For INTERNAL operations — consult the relevant `[Pillar].proposals_ledger.yaml` under `tracked_gaps` before starting.

7. **MANDATORY:** Update `system-board.yaml` at the start AND end of every operation cycle.
