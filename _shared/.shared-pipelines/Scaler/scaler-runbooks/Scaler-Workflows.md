...
credentials:
  description: Phase-by-phase execution playbook, per-phase reference cards, audit
    pass procedure, and 7 audit checks
...
- **Step 7: Aspect & Pending Linking**: Update the existing `[Pillar].sources_ledger.yaml` entry (created during Phase 1) with the newly resolved `primary_aspect` and `aspects[]` list. Multi-pillar items update each respective pillar's entry. The auto-sync re-aggregates the `system-board.yaml` rollup.

### Phase 3: Capability Engineering
- **Assess**: Determine if new or enhanced agentic skills, tools, or toolboxes from `_shared/.shared-toolboxes/` are required to architect the solution.
...
- **Sync**: Update `system-board.yaml` and all needed scaler related files (e.g. `system-board.yaml`) and trigger `.infra/backend/engine.py` to self-heal the system map.
...
2. **Discovery Scan**: Scan typed pillar folders for new, unprocessed functional groups. Read each group as a single unit; walk sub-groups bottom-up so the full picture is loaded before drafting. Sub-grouping depth is unbounded.
    - **EXTERNAL PROMPT**: If `scaler.input_mode: EXTERNAL` (or `AUTO` resolving to `EXTERNAL`), the agent MUST follow the active `system.action_gate`. If `EXECUTION`, run the analysis prompt immediately. If `PLANNING`, the executing agent MUST first ask the user for explicit approval (via direct prompt or system-board.yaml). Once approved (or if in EXECUTION mode), the agent MUST run the following external analysis prompt verbatim to drive the discovery work:
...
7. **Mode-Aware Integration**:
   ...
   - `PLANNING` mode → Post review request in `system-board.yaml` communication block. Await user approval.
...
3. **Capability Engineering**: Utilize `_shared/.shared-toolboxes/` tools for planning and logic engineering.
...
5. **Mode-Aware Integration**:
   ...
   - `PLANNING` mode (via global `system.action_gate`) → Post review request in `system-board.yaml` communication block. Await user approval.

---

## 4. The Execution & Tracking Rule
- **Milestones vs. Scaler Ledgers**: The milestones (in `.missions/`) track the High-Level Goal. The `scaler_ledgers/` inside `.meta/.os/pipeline_scaler.runbooks/` act as the deep, granular ledgers mapping out those actual files, their current paths, and processing states.
...
| **RESTRUCTURE_ARCHITECTURE** | Discovery reveals that existing folder structure, naming, or system relationships need reorganization. **Always requires explicit user approval in ALL modes.** Post in `system-board.yaml` before proceeding. | Splitting `identity` into sub-aspects; reorganizing the toolbox hierarchy |
...
- **Architecture changes need approval**: If the analysis concludes that the OS architecture itself must change to accommodate the discovery (`ARCHITECTURE_AUDIT`), this ALWAYS requires explicit user approval regardless of `action_gate` mode. Post in `system-board.yaml` communication block.
...
- **Inputs**:
  ...
  - For INTERNAL: `_system/.system-meta/.system-os_prompts/`, `system-board.yaml`, `_shared/.shared-toolboxes/`, `system-board.yaml`
- **Outputs**:
...
- **Inputs**:
  ...
  - Toolbox catalog `_shared/.shared-toolboxes/` and router `system-board.yaml`.
...
- **Inputs**:
  ...
  - Active .system.board profile (`INTERNAL` or `EXTERNAL`).
...
- **Outputs**:
  ...
  - Review-queue entry in `system-board.yaml.communication_hubs.scaler_hub.scaler_review_queue` if PLANNING.
- **Error Recovery**:
  | Failure mode | Action |
  |---|---|
...
- **Outputs**:
  ...
  - Post-integration sync: `system-board.yaml`, `system-board.yaml.last_sync` + `recent_events`, `system-board.yaml` alignment via `.infra/backend/engine.py`.
...
- **Error Recovery**:
  ...
  | `.infra/backend/engine.py` fails after integration | Card stays `PENDING_INTEGRATION`; surface to scaler_hub.messages; never claim INTEGRATED with broken router |
...
- **Manual trigger**: User sets `system-board.yaml.modes.scaler.audit_request: true`. The Scaler picks it up at the next cycle start.
- **Goal-completion trigger**: When a Scaler-related goal is marked `done` in `.missions/`, the Scaler MAY queue an Audit Pass for the next cycle (configurable via `system-board.yaml.audit_policy`).
...
- `system-board.yaml.communication_hubs.scaler_hub.recent_events` with the audit summary.
...
| 5 | **Router freshness** | Compare central ledgers in `index.yaml` against live disk state. Any mismatch indicates a missed `.infra/backend/engine.py` run after a recent integration. |
...
1. **Pre-flight**: Confirm P-LAW-008 runbook immersion is fresh; lock `system-board.yaml.state.audit_in_progress: true` to prevent concurrent cycles from conflicting.
...
- The Audit Pass is **bounded in time**: if any check exceeds the `audit_check_timeout` configured in `system-board.yaml` (default 5 minutes per check), it is logged as `INCOMPLETE` and the cycle continues to the next check.
- Findings remain in `system-board.yaml.state.audit_findings[]` for at least 1 quarter (rotates with `pipelines_runtime/.scaler_runtime/.scaler_archive/`) for traceability.
