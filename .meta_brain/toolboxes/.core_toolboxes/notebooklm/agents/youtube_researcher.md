...
If instructed by the CONTROLER or mission, you may also use the `notebooklm ask` or `notebooklm download` commands to extract the ingested scripts as raw text files and drop them into designated OS staging folders (e.g., `pipelines/scaler/EXTERNAL/discoveries/.mixed/.inbox/`).
