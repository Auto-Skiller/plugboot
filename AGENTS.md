...
- BOOT-6 Reconcile & propose (first boot, and every boot): cross-reference `config.yaml` armed toggles (any `auto_triggering` / `auto_execution` = true) against the scanned entity state from BOOT-5. Output a prioritized **next-actions suggestion**. Rules for this step — NON-NEGOTIABLE:
  - **Backlog is the user's priority intent.** Read `runtime.backlog` first; its items are the top-priority next actions unless a critical issue is found elsewhere (missing fields, schema/sync break, metric-vs-disk mismatch). Critical issues outrank backlog; everything else yields to it.
- **Enumerate EVERY entry**, never summarize or sample.
...
- `project_name/` — one folder per project. Self-contained: board, runtime, missions, toolboxes, inbox (raw drops + `.<entity>-inbox_gateway/` with pillars -> functional groups), and a freeform `*-data/` folder described by `*-data.yaml`.
