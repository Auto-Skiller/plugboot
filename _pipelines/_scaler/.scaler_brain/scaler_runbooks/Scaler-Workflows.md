...
- **INTERNAL**: Audit the top-layer OS components (`.identity/`, `meta.router/`, `.toolbox_library/`, etc.) to identify structural, capability, or business gaps.
  - **EVOLVE Prompt**: If `system.evolution_mode` is `EVOLVE` and `scaler.input_mode` is `INTERNAL` (or `AUTO` resolving to `INTERNAL`), the agent MUST ask for explicit user approval before executing the comprehensive systemic analysis prompt defined in Section 3.
...
- **Sync**: Update `.meta_brain/meta_router.yaml` and all needed scaler related files (e.g. SCALER-STATE.yaml) and trigger `meta_sync.py` to self-heal the system map.
