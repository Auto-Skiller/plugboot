...
- **Step 7: Track (sources_ledger first)**: Log in `[Pillar].sources_ledger.yaml` (anti-duplication via content hash). Multi-pillar items get one entry per pillar, all sharing `multi_pillar_ref_id` and listing each other under `multi_pillar_siblings`. The auto-sync re-aggregates the `.scaler_routing/scaler_ledgers.yaml` rollup.
...
4. **Tracking (sources_ledger first)**: Log the group's items in the relevant `[Pillar].sources_ledger.yaml` (inside `scaler_ledgers/`) for anti-duplication. Multi-pillar items get one entry per pillar, all sharing `multi_pillar_ref_id`. The auto-sync re-aggregates `.scaler_routing/scaler_ledgers.yaml` rollup. Items rejected to `.complex_inboxes/` get a `rejected_to_complex_inboxes: true` marker in the originating ledger (mixed_inbox or typed inbox source).
