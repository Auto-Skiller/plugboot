...
> This protocol closes the loop between Scaler discovery and `.toolbox_library/` population.
...
integration_strategy:
  ...
  toolbox_target:
    toolbox_path: ".brain/.toolbox_library/extended.toolbox/engineering.toolbox/coding"
    toolbox_yaml: ".brain/.toolbox_library/extended.toolbox/engineering.toolbox/coding/coding.yaml"
  ...
  execution_plan:
    ...
    - step: 4
      action: "RUN sync_engine → toolbox_library.router.yaml health block auto-updates"
    - step: 5
...
1. Run `sync_engine.py` to remap `toolbox_library.router.yaml`.
2. Verify `health.status` block updated (empty → partial → functional).
3. Add entry to `CHANGELOG.md` in `.brain/.toolbox_library/`.
