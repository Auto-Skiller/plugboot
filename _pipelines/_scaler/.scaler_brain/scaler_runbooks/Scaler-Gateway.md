...
- **EXTERNAL**: Add entry to the relevant `[Pillar].sources_ledger.yaml.state.tracked_discoveries[]` (with content hash). The auto-sync rolls this into `.scaler_routing/scaler_ledgers.yaml`.
- **INTERNAL (gap)**: Add entry to the relevant `[Pillar].proposals_ledger.yaml` under `state.tracked_gaps[]`.
...
4. New cross-toolbox edges are added via UPGRADE cards under the
   `dependency_graph.edges` block of `.meta_brain/.meta_routing/toolboxes.yaml`.
