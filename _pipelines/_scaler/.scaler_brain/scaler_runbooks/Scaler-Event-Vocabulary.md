...
An event belongs in this file if and only if it is emitted from somewhere inside `_pipelines/_scaler/`. That includes:
...
| `CARD_INTEGRATED` | Phase 5 finishes all `files_involved` actions | `card_id`, `files_touched[]`, `at` | ledger, `scaler_state.metrics` |
...
| Event | Emitted by | Payload | Consumer |
|---|---|---|---|
| `AUDIT_STARTED` | Audit Pass acquires the lock | `triggered_by` (`manual|goal_complete|drift_suspected|quarter_rotation`), `at` | `scaler_state.audit_in_progress: true` |
| `AUDIT_CHECK_RAN` | One of the 6 checks completes | `check_id` (1..6), `findings_count`, `severity_breakdown`, `at` | `scaler_state.audit_findings[]` |
...
| `AUDIT_COMPLETED` | All 6 checks done; lock released | `outcome` (`CLEAN|WARN|DRIFT|INCOMPLETE`), `at` | `scaler_hub.recent_events` |

## 8. Sync Events (Scaler-internal — distinct from OS-wide `META_SYNC_*`)

| Event | Emitted by | Payload | Consumer |
|---|---|---|---|
| `STATE_SYNC` | `scaler_state.yaml` mirrors CONTROLER pre-cycle (P-LAW-007) | `mirrored_fields[]`, `at` | scaler_state |
| `LEDGER_ROLLUP_REASSEMBLED` | Auto-aggregator rebuilt `.scaler_routing/scaler_ledgers.yaml` | `pillars_aggregated[]`, `at` | router |
| `RUNTIME_ROLLUP_REASSEMBLED` | Auto-aggregator rebuilt `.scaler_routing/scaler_runtime.yaml` | `at` | router |
| `RUNBOOK_READINESS_LOGGED` | P-LAW-008 acknowledgement (when enforced) | `runbook_file`, `content_hash`, `at` | `scaler_state.runbook_readiness` |
...
2. **No emission to Hustler stores.** Scaler events are written to `scaler_state.yaml`, `scaler_hub` (inside CONTROLER's communication_hubs), or per-pillar ledgers. Never to `hustler_state.yaml`, `hustler_hub`, or any Hustler ledger.
...
5. **Audit-Pass emissions are observation-only.** Audit events do not cause writes outside `scaler_state.yaml` and the audit-remediation Mega-YAML — they describe state, never change it.
