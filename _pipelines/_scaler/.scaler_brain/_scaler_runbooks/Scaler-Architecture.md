...
- `_scaler_runbooks/`: The operational rules and workflows for scaling that need to be strictly read before any scaler execution.
...
- **Master EXTERNAL_INBOX-LEDGER** (`scaler_ledgers/EXTERNAL_INBOX-LEDGER.yaml`): Aggregates counts from sub-ledgers + directly tracks D-level (parent) discoveries only. Does NOT duplicate sub-ledger detail.
