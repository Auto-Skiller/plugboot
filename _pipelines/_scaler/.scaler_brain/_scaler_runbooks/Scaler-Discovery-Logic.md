...
|---|---|
| D (parent discovery) | Sub-ledger (`[Pillar].ledger.yaml`) **AND** master `EXTERNAL_INBOX-LEDGER.yaml` `tracked_discoveries` |
...
- **Action**: Move the discovery folder/files from the active pillar folder (e.g., `_Foundational_Integrity/`, `_Operational_Muscles/`) to `.scaler_runtime/.scaler_archive/EXTERNAL/discoveries/`.
