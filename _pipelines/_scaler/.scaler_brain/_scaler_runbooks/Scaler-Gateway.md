...
> **PREVENTION RULE**: Before creating any card, check `scaler_state.yaml → gateway_metrics.pending_approvals_count`. If > 0, address or acknowledge pending cards first before creating new ones (unless unrelated scope).
...
|------|---------------|----------------|
| **Proposal Card** | Any EXTERNAL output (direct moves, adaptations, partial extracts, architecture audits) | `[Pillar]/external_proposals/PROP-EXT-[ID].yaml` |
| **Internal Action Card (Mega-YAML)** | Any INTERNAL gap and solution combined | `[Pillar]/internal_proposals/MEGA-INT-[ID].yaml` |
...
1. Read `scaler_state.yaml` → confirm current phase and gateway metrics.
2. Read the master `EXTERNAL_INBOX-LEDGER.yaml` or the relevant `[Pillar].ledger.yaml` (inside `scaler_ledgers/`) → confirm no duplicate processing (check `processed_matrix` or logged discoveries/gaps).
3. For EXTERNAL: check all pending proposals for a `MERGE_WITH_PENDING` match.
4. Update `scaler_state.yaml → state.current_phase` to the appropriate phase.
...
- **EXTERNAL**: Add entry to the relevant pillar sub-ledger AND `EXTERNAL_INBOX-LEDGER.yaml → tracked_discoveries[]`.
- **INTERNAL (gap)**: Add entry to the relevant pillar's sub-ledger (`[Pillar].ledger.yaml` under `state.tracked_gaps[]`).
- Set `integration_status: PENDING`.
- Update `scaler_state.yaml → gateway_metrics.pending_approvals_count` (increment by 1 if PLANNING mode).
- Update `scaler_state.yaml → gateway_metrics.last_gateway_action`.
...
- Post a review request in `CONTROLER.yaml → communication.scaler_review_queue`:
  ...
  - id: [card_id]
    ...
    status: PENDING
  ```
- Update `scaler_state.yaml → active_triage[]` with the card.
...
5. Update `.meta_brain/milestones/` goal `artifacts[]` with the integrated file paths.
...
- **PROPOSALS**: Move to `.scaler_runtime/.scaler_archive/EXTERNAL/[Pillar]/external_proposals/`
- **INTERNAL**: Move to `.scaler_runtime/.scaler_archive/INTERNAL/[Pillar]/internal_proposals/`
...
- [ ] `scaler_state.yaml` updated (phase, metrics, gateway_metrics)
...
> This protocol closes the loop between Scaler discovery and `.meta_brain/toolboxes/` population.
...
integration_strategy:
  ...
  toolbox_target:
    toolbox_path: ".meta_brain/toolboxes/engineering/coding"
    toolbox_yaml: ".meta_brain/toolboxes/engineering/coding/coding.yaml"
...
1. Run `meta_sync.py` to remap `toolboxes_router.yaml`.
2. Verify `health.status` block updated (empty → partial → functional).
3. Add entry to `CHANGELOG.md` in `.meta_brain/toolboxes/`.
4. Update `toolbox_graph.yaml` if the skill creates a new dependency edge.
