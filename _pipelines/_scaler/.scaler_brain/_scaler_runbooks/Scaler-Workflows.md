...
- **Step 6: Track (sub-ledger first)**: Log in `[Pillar].ledger.yaml` then update `EXTERNAL_INBOX-LEDGER.yaml`.
...
4. **Tracking (sub-ledger first)**: Log the discovery in the relevant `[Pillar].ledger.yaml` (inside `scaler_ledgers/`). Then update master `EXTERNAL_INBOX-LEDGER.yaml` rollup. D-level discoveries also get an entry in the master `tracked_discoveries` list.
...
- **Anti-Duplication**: When logging an external file in `EXTERNAL_INBOX-LEDGER.yaml`, append the target aspect and level to the `processed_matrix`. The scaler MUST NOT process the exact same file for the exact same aspect and level twice.
