...
> **Isolation contract**: Per the workspace's pipeline-isolation principle (see `Hustler-Operational-Rules.md §5` Self-Evolution Protocol), the Scaler and Hustler do not share an event bus, do not share a log file, and do not reference each other's vocabularies. The OS-wide event vocabulary at `.meta_brain/meta_identity/Event_Vocabulary.md` covers events emitted *outside* both pipelines (CONTROLER, milestones, meta_sync) and is the only shared catalog.

---

## 1. Scope

An event belongs in this file if and only if it is emitted from somewhere inside `_pipelines/hustler/`. That includes:
- Hustler agents performing Phase 1-5 cascade and processing work
- `hustler_sync.py` and its sub-sync engines
...
| `FOCUS_VALIDATED` | Focus threshold met; new focus folder + split ledgers created | `focus_id`, `triggering_sources[]`, `at` | hustler_ledgers, `hustler_hub.recent_events` |
...
| `HUSTLE_SESSION_OPENED` | New `HUSTLE-[Market]-[ID]` session opened (H-LAW-001) | `hustle_id`, `market`, `product_ref`, `roi_projection_ref`, `at` | hustler_ledgers, milestones |
...
| `PARITY_AUDIT_RAN` | H-LAW-014 step 1 completed for a retiring product/feature | `level`, `before_id`, `dependent_artifacts[]`, `at` | `hustler_state.rescoping_history[]` |
...
| Event | Emitted by | Payload | Consumer |
|---|---|---|---|
| `AUDIT_STARTED` | Audit Pass acquires the lock | `triggered_by` (`manual|goal_complete|drift_suspected|quarter_rotation`), `at` | `hustler_state.audit_in_progress: true` |
| `AUDIT_CHECK_RAN` | One of the 6 checks completes | `check_id` (1..6), `findings_count`, `severity_breakdown`, `at` | `hustler_state.audit_findings[]` |
...
| Event | Emitted by | Payload | Consumer |
|---|---|---|---|
| `STATE_SYNC` | `hustler_state.yaml` mirrors CONTROLER pre-cycle | `mirrored_fields[]`, `at` | hustler_state |
| `LEDGER_ROLLUP_REASSEMBLED` | Auto-aggregator rebuilt `.hustler_routing/hustler_ledgers.yaml` | `focuses_aggregated[]`, `at` | router |
| `RUNTIME_ROLLUP_REASSEMBLED` | Auto-aggregator rebuilt `.hustler_routing/hustler_runtime.yaml` | `at` | router |
| `RUNBOOK_READINESS_LOGGED` | H-LAW-010 acknowledgement (when enforced) | `runbook_file`, `content_hash`, `at` | `hustler_state.runbook_readiness` |

---

## 10. Severity & Format

Severities follow the OS-wide ladder (`INFO|WARN|ERROR|CRITICAL` per `.meta_brain/meta_identity/Event_Vocabulary.md §2.4`). Format also follows the OS-wide canonical string for `recent_events` and structured form for `messages[]`. Using the OS ladder keeps Hustler events readable when an external observer scans the merged audit trail in `CONTROLER.yaml`.
...
2. **No emission to Scaler stores.** Hustler events are written to `hustler_state.yaml`, `hustler_hub` (inside CONTROLER's communication_hubs), or per-focus ledgers/trackers. Never to `scaler_state.yaml`, `scaler_hub`, or any Scaler ledger.
...
5. **Audit-Pass emissions are observation-only.** Audit events do not cause writes outside `hustler_state.yaml` and the remediation message — they describe state, never change it.
