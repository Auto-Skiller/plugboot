...
| 🔄 **Pipeline Blueprints** | Pipelines are definitions, not folders. They execute inside each entity's `pipelines_runtime/`. |
...
Pipelines are **blueprints** (definitions + runbooks + contracts) that live in a shared or entity-specific library. They execute **inside the consuming entity's runtime folder**.
...
Pipeline Execution (runtime):
  _system/.system-pipelines_runtime/      ← Scaler running for system work
  project_foo/.project_foo-pipelines_runtime/  ← Scaler running for project_foo
```
