...
> AI design contract generated by `/ai-integration-phase`. Consumed by `planner` and `eval-auditor`.
...
> Researched by `domain-researcher`. Grounds the evaluation strategy in domain expert knowledge.
...
> Fetched from official docs by `ai-researcher`. Distilled for this specific use case.
...
> Written by `ai-researcher`. Cross-cutting patterns every developer building AI systems needs — independent of framework choice.
