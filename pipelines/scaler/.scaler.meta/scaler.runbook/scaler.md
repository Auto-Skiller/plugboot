...
> - `\.brain\pipelines-brain\scaler_brain\discovery_to_proposal.md`
...
5. **Integration Rule**: When discovering and integrating new capabilities or opportunities, prioritize **copying** them exactly as they are if possible. Only 'turn' (refactor or heavily modify) them if they cannot work directly within our architecture.
6. **Proposal Routing**: Convert discoveries into formal proposals and route them to `_proposals/` subfolders (Brain, Core, Departments, Tools, etc.) as defined in `proposals.json`.
