...
- **Toolbox Usage (Dynamic Detection)**: The Hustler MUST invoke toolboxes from `.toolboxes/` for all analysis/extraction work (PDF parsing, web scraping, content classification, ranking, summarization, etc.). **There is no static mapping of phase → toolbox.** The agent detects the relevant toolbox per task by consulting the live toolbox catalog (`.toolboxes/`) and the auto-generated toolbox router (`.meta_os/meta_db/.toolboxes.yaml`). This keeps the pipeline forward-compatible: as new toolboxes are added, the Hustler picks them up automatically without runbook changes.
...
- **Goal-completion trigger**: When a Hustler-related goal is marked `done` in `.milestones/`, the Hustler MAY queue an Audit Pass for the next cycle (configurable via `.meta_os/meta_db/pipeline_hustler_os.yaml.audit_policy`).
...
- `.meta_os/meta_db/pipeline_hustler_os.yaml.state.last_audit` + `audit_findings[]`
...
1. **Pre-flight**: Confirm H-LAW-010 runbook immersion is fresh; lock `.meta_os/meta_db/pipeline_hustler_os.yaml.state.audit_in_progress: true` to prevent concurrent cycles.
...
3. **Aggregate findings**: append to `.meta_os/meta_db/pipeline_hustler_os.yaml.state.audit_findings[]` with the run timestamp.
4. **Decision**:
...
- The Audit Pass is **bounded in time**: if any check exceeds the `audit_check_timeout` configured in `.meta_os/meta_db/pipeline_hustler_os.yaml` (default 5 minutes per check), it is logged as `INCOMPLETE` and the cycle continues.
- Findings remain in `.meta_os/meta_db/pipeline_hustler_os.yaml.state.audit_findings[]` for at least 1 quarter (rotates with `.hustler_archive/`) for traceability.
