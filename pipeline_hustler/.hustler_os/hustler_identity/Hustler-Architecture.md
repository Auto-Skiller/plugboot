...
- `meta_identity/`: Core identity, routing rules, and execution flow (`.meta_os/meta_identity/`).
...
- `.meta_os/meta_db/pipeline_hustler_os.yaml`: The localized DB configuration file in `.meta_os/meta_db/` — absolute tracking point for the pipeline state.
- `.toolboxes/`: Core agentic and extended capabilities. Use specific toolboxes for analytics, planning, content extraction.
...
|---|---|---|
| The global `.meta/engine/meta_sync.py` engine aggregates totals into central `.meta_os/meta_db/` rollups | All validated focuses + master aggregates | Total focuses/products/features counts, pending in `.hustler_mixed_inbox/` |
...
- **`pipeline_hustler_state`**: State management housed in `.meta_os/meta_db/pipeline_hustler_os.yaml`.
- **`hustler_ledgers`**: Granular per-focus sub-ledgers in `pipeline_hustler/.hustler_os/hustler_db/`, which are automatically aggregated into central `.meta_os/meta_db/` rollups.
