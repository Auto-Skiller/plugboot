...
- `meta_identity/`: Core identity, routing rules, and execution flow (`.identity/`).
...
- `.db/pipeline_hustler.yaml`: The localized DB configuration file in `.db/` — absolute tracking point for the pipeline state.
- `_toolboxes/`: Core agentic and extended capabilities. Use specific toolboxes for analytics, planning, content extraction.
...
│   │
│   └── `.hustler_db/`                             # 🗂️ Centralized tracking ledgers
...
│
└── [focus-name]/                                  # ✅ A VALIDATED Focus folder (sibling to .hustler_identity/)
...
> **Note on the layout:** The validated focus folder lives **at the pipeline root** (sibling to `.hustler_identity/`). All Phase 3-5 build work happens here. Tracking lives separately in `.hustler_db/[focus].focus_ledger.yaml` (strategic) + `[focus].sources_ledger.yaml` (anti-duplication). The brain tracks; the root focus folder builds.
...
|---|---|---|
| The global `_os/engine/meta_sync.py` engine aggregates totals into central `.db/` rollups | All validated focuses + master aggregates | Total focuses/products/features counts, pending in `.hustler_mixed_inbox/` |
...
| `[focus].focus_ledger.yaml` (in `.hustler_db/`) | Per-focus strategic rollup — products/features + market context | Products + features per focus, currency/language/delivery/payment context |
| `[focus].sources_ledger.yaml` (in `.hustler_db/`) | Per-focus anti-duplication tracker for inbound cascades | Content hashes of sources cascaded into the focus |
...
|---|---|---|---|
| `.hustler_identity/` | **Logic, routing, runbooks** | Runbooks, state configurations | Active discoveries, scraped data, ephemeral files |
...
- **`pipeline_hustler_state`**: State management housed in `.db/pipeline_hustler.yaml`.
- **`hustler_ledgers`**: Granular per-focus sub-ledgers in `pipeline_hustler/.hustler_db/`, which are automatically aggregated into central `.db/` rollups.
