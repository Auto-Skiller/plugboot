...
> **Known limitation (audit-acknowledged).** Like Scaler P-LAW-008, this rule is currently honor-system: no programmatic gate verifies the read. A future enhancement could store per-session runbook acknowledgements (timestamp + content-hash) in `.db/pipeline_hustler.yaml.runbook_readiness` and enforce pre-flight refusal on missing/stale acks. Until enforced, agents are bound by H-LAW-010 on trust.
...
- **Pre-Flight Check**: Verify `.db/pipeline_hustler.yaml` exists and has a valid current_phase before initiating sync.
...
Every re-scoping operation appends one entry to `.db/pipeline_hustler.yaml.state.rescoping_history[]` with: `level: focus|product|feature`, `before_id`, `after_id` (or null for retirement), `reason`, `dependent_artifacts`, `timestamp`.
...
1. **Pillar Dominance**: The identity context (`_context/.identity/`) rules always take precedence. Hustler logic adapts; system laws do not.
2. **Deterministic Precedence**: The Master DB Index (`.db/DB.yaml`) is the final authority. If a Hustler operation contradicts the master index, the operation is rejected pending an Internal Action Card via the Scaler.
...
4. **MANDATORY:** Update `.db/pipeline_hustler.yaml` at the start AND end of every operation cycle.
