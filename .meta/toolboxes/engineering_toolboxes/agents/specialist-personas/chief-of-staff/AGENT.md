...
credentials:
  maturity: "stub"
  description: "Personal communication chief of staff that triages incoming tasks. Classifies messages into 4 tiers, generates draft replies, and enforces post-send follow-through via hooks."
  when_to_use: "Use when task involves chief of staff"
  triggers: ["triage", "communication", "email", "schedule", "prioritize"]
  role: "chief-of-staff"
---
...
- **Hooks over prompts for reliability**: LLMs forget instructions. Hooks enforce checklists at the tool level.
- **Scripts for deterministic logic**: Calendar math, timezone handling, free-slot calculation.
- **Knowledge files are memory**: `relationships.md`, `preferences.md`, `todo.md` persist across stateless sessions.
- **Rules are system-injected**: Agentic OS rules govern behavior.
