...
2. **Discovery Scan**: Scan typed pillar folders for new, unprocessed functional groups. Read each group as a single unit; walk sub-groups bottom-up so the full picture is loaded before drafting. Sub-grouping depth is unbounded.
    - **EXTERNAL PROMPT**: If `scaler.input_mode: EXTERNAL` (or `AUTO` resolving to `EXTERNAL`), the agent MUST follow the active `system.action_gate`. If `EXECUTION`, run the analysis prompt immediately. If `PLANNING`, the executing agent MUST first ask the user for explicit approval (via direct prompt or .db/.system.board.yaml). Once approved (or if in EXECUTION mode), the agent MUST run the following external analysis prompt verbatim to drive the discovery work:
...
7. **Mode-Aware Integration**:
   ...
   - `PLANNING` mode → Post review request in `.db/.system.board.yaml` communication block. Await user approval.
...
5. **Mode-Aware Integration**:
   ...
   - `PLANNING` mode (via global `system.action_gate`) → Post review request in `.db/.system.board.yaml` communication block. Await user approval.

---

## 4. The Execution & Tracking Rule
- **Milestones vs. Scaler Ledgers**: The milestones (in `.db/milestones/`) track the High-Level Goal. The `scaler_ledgers/` inside `.meta/.os/pipeline_scaler.runbooks/` act as the deep, granular ledgers mapping out those actual files, their current paths, and processing states.
...
| **RESTRUCTURE_ARCHITECTURE** | Discovery reveals that existing folder structure, naming, or system relationships need reorganization. **Always requires explicit user approval in ALL modes.** Post in `.db/.system.board.yaml` before proceeding. | Splitting `identity` into sub-aspects; reorganizing the toolbox hierarchy |
...
- **Architecture changes need approval**: If the analysis concludes that the OS architecture itself must change to accommodate the discovery (`ARCHITECTURE_AUDIT`), this ALWAYS requires explicit user approval regardless of `action_gate` mode. Post in `.db/.system.board.yaml` communication block.
...
- **Outputs**:
  ...
  - Review-queue entry in `.db/.system.board.yaml.communication_hubs.scaler_hub.scaler_review_queue` if PLANNING.
- **Error Recovery**:
  | Failure mode | Action |
  |---|---|
...
- **Goal-completion trigger**: When a Scaler-related goal is marked `done` in `.db/milestones/`, the Scaler MAY queue an Audit Pass for the next cycle (configurable via `.db/pipeline_scaler.board.yaml.audit_policy`).
...
- `.db/pipeline_scaler.board.yaml.state.last_audit` + `audit_findings[]`
- A new INTERNAL Mega-YAML in `[Pillar]_internal_proposals/` IF drift is detected (named `MEGA-INT-AUDIT-REMEDIATION-[timestamp]`).
- `.db/.system.board.yaml.communication_hubs.scaler_hub.recent_events` with the audit summary.
...
1. **Pre-flight**: Confirm P-LAW-008 runbook immersion is fresh; lock `.db/pipeline_scaler.board.yaml.state.audit_in_progress: true` to prevent concurrent cycles from conflicting.
...
3. **Aggregate findings**: append to `.db/pipeline_scaler.board.yaml.state.audit_findings[]` with the run timestamp.
4. **Decision**:
