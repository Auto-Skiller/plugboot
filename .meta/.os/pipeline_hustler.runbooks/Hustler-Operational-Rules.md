---
metadata:
  name: hustler-operational-rules
...
credentials:
  description: 15 H-LAWs governing Hustler behavior — cascading integrity, atomic
    tracker update, DNA preservation, source quality bar, and conflict resolution
  when_to_use: Read when making cascade decisions — defines all operational constraints
    and the 15 H-LAWs
  contains: h_laws, atomic_tracker, operational_rules
---
...
- **Mandatory Gateway**: Every cascade decision in `PLANNING` mode MUST be materialized as a review request in `.db/.system.board.yaml.communication_hubs.hustler_hub.messages` before any move is committed.
...
4. **Log the failure** to `.db/.system.board.yaml.communication_hubs.hustler_hub.messages` with:
...
> **Known limitation (audit-acknowledged).** Like Scaler P-LAW-008, this rule is currently honor-system: no programmatic gate verifies the read. A future enhancement could store per-session runbook acknowledgements (timestamp + content-hash) in `.db/pipeline_hustler_os.yaml.runbook_readiness` and enforce pre-flight refusal on missing/stale acks. Until enforced, agents are bound by H-LAW-010 on trust.

**Enforced:** false (honor-system; see caveat).

#### H-LAW-011 — Mode-Aware Action Gate
The Hustler MUST consult `.db/.system.board.yaml.modes.hustler.action_gate` before any cascade or processing action. If `action_gate: EXECUTION`, the Hustler proceeds autonomously. If `action_gate: PLANNING` (default), the Hustler MUST post the proposed action to `communication_hubs.hustler_hub.messages` and wait. (Mirror of Scaler P-LAW-018.)
**Enforced:** true.
...
- **Pre-Flight Check**: Verify `.db/pipeline_hustler_os.yaml` exists and has a valid current_phase before initiating sync.
...
- If the action is in `PLANNING` → post a review request in `.db/.system.board.yaml.communication_hubs.hustler_hub.messages` and wait.
...
Every re-scoping operation appends one entry to `.db/pipeline_hustler_os.yaml.state.rescoping_history[]` with: `level: focus|product|feature`, `before_id`, `after_id` (or null for retirement), `reason`, `dependent_artifacts`, `timestamp`.
...
1. **Pillar Dominance**: The identity context (`.meta/.os/.system.identity/`) rules always take precedence. Hustler logic adapts; system laws do not.
2. **Deterministic Precedence**: The Master DB Index (`.db/.system.board.yaml`) is the final authority. If a Hustler operation contradicts the master index, the operation is rejected pending an Internal Action Card via the Scaler.
...
4. **MANDATORY:** Update `.db/pipeline_hustler_os.yaml` at the start AND end of every operation cycle.
