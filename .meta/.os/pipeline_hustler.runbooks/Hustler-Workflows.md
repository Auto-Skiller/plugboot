...
- Apply the cascading decision tree in `Hustler-Cascading-Logic.md`:
  1. **Focus check** against the central `.db/` rollups (rolled up from per-focus `[focus].focus_ledger.yaml` files).
...
4. **Mode-Aware Integration**:
   ...
   - `PLANNING` mode → leave the proposed cascade in scratch with a review request in `.db/.system.board.yaml.communication_hubs.hustler_hub`.
...
- **Toolbox Usage (Dynamic Detection)**: The Hustler MUST invoke toolboxes from `.meta/toolboxes/` for all analysis/extraction work (PDF parsing, web scraping, content classification, ranking, summarization, etc.). **There is no static mapping of phase → toolbox.** The agent detects the relevant toolbox per task by consulting the live toolbox catalog (`.meta/toolboxes/`) and the auto-generated toolbox router (`.db/toolboxes.board.yaml`). This keeps the pipeline forward-compatible: as new toolboxes are added, the Hustler picks them up automatically without runbook changes.
...
The `.db/.system.board.yaml.modes.hustler` block controls Hustler behavior:
...
| `action_gate: PLANNING` | Cascade and processing proposals are drafted but await user approval via `.db/.system.board.yaml.communication_hubs.hustler_hub.messages`. |
...
- **Inputs**:
  ...
  - Central `.db/` rollups (auto-rolled from per-focus ledgers).
...
- **Manual trigger**: User sets `.db/.system.board.yaml.modes.hustler.audit_request: true`. The Hustler picks it up at the next cycle start.
- **Goal-completion trigger**: When a Hustler-related goal is marked `done` in `.milestones/`, the Hustler MAY queue an Audit Pass for the next cycle (configurable via `.db/pipeline_hustler_os.yaml.audit_policy`).
...
- `.db/pipeline_hustler_os.yaml.state.last_audit` + `audit_findings[]`
- A remediation message in `.db/.system.board.yaml.communication_hubs.hustler_hub.messages` IF drift is detected.
- `.db/.system.board.yaml.communication_hubs.hustler_hub.recent_events` with the audit summary.
...
1. **Pre-flight**: Confirm H-LAW-010 runbook immersion is fresh; lock `.db/pipeline_hustler_os.yaml.state.audit_in_progress: true` to prevent concurrent cycles.
...
3. **Aggregate findings**: append to `.db/pipeline_hustler_os.yaml.state.audit_findings[]` with the run timestamp.
4. **Decision**:
...
- The Audit Pass is **bounded in time**: if any check exceeds the `audit_check_timeout` configured in `.db/pipeline_hustler_os.yaml` (default 5 minutes per check), it is logged as `INCOMPLETE` and the cycle continues.
- Findings remain in `.db/pipeline_hustler_os.yaml.state.audit_findings[]` for at least 1 quarter (rotates with `.hustler_archive/`) for traceability.
