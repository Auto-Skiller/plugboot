...
Controlled via `.db/.system.board.yaml` configuration.
...
- **Foundational_Integrity** (Goal: Stability): Consult `.db/.system.board.yaml` and the pipelines router at `.db/pipeline_scaler.board.yaml`. Read relevant laws in `.meta/.os/.system.identity/` and workflow logic in `pipeline_scaler/.scaler_os/scaler_identity/`.
- **Operational_Muscles** (Goal: Power): Consult `.db/toolboxes.board.yaml`. Read relevant `yaml_path` entries to see if discovery matches existing descriptions or triggers.
...
| `mission_board` | Session and goal tracking files, runtime state | `.db/meta_milestones/` |
...
> **SCOPE CREATION LAW**: The Scaler MUST suggest any new scopes (aspects) in the `.db/system_status.scope_suggestions[]` block. If `system.action_gate` is `EXECUTION`, the Scaler may proceed with the creation if the task is part of an approved session. If `PLANNING`, it MUST await explicit user approval.
...
The `action_gate` in `.db/.system.board.yaml` is controlled via **Profiles** (`INTERNAL` | `EXTERNAL`). Each profile contains lists that define how the Scaler behaves per `Integration_Type`:
...
| **PLANNING** | If the `integration_type` is found in the `PLANNING` list, it **stays in the folder**. The Scaler posts a review request in the `.db/.system.board.yaml`. Integration only happens after explicit user approval. |

**Selection Logic:**
- The Scaler determines the source of the operation (`INTERNAL` or `EXTERNAL`).
- It loads the corresponding profile from `.db/.system.board.yaml`.
...
- **`scaler_ledgers`**: Granular per-pillar sub-ledgers in `pipeline_scaler/.scaler_os/scaler_db/`, which are automatically aggregated into central `.db/` rollups.
