...
- **Atomic State**: Every operation must be preceded by a state update in `.db/pipeline_scaler_os.yaml`. You MUST explicitly synchronize `.db/pipeline_scaler_os.yaml` (`active_mode` block) with `.db/.system.board.yaml` prior to acting to prevent configuration drift.
...
- **No Scope Creation Without Approval**: The Scaler MUST NOT create new scopes (aspects) autonomously. New scope suggestions must be posted in the `.db/.system.board.yaml` communication block. Integration only proceeds after explicit user approval — regardless of the active `action_gate` mode.
...
1. **Pillar Dominance**: The identity context (`.meta/.os/.system.identity/`) rules always take precedence. You can adopt the discovery to become compatible with the current architecture.
2. **Deterministic Precedence**: The Master DB Index (`.db/.system.board.yaml`) is the final authority. If a proposal contradicts the Index without a plan for a safe migration, it must be rejected. We need an adopting plan for a safe migration.
...
The `action_gate` setting in `.db/.system.board.yaml` governs Scaler behavior AFTER the gateway checkpoint:
...
- The .db/.system.board.yaml communication block is updated post-integration to inform the user.
...
### PLANNING Mode
- After drafting a Proposal/Solution Card in the gateway folder, the Scaler STOPS and posts a review request in the `.db/.system.board.yaml` communication block.
...
1. `.db/.system.board.yaml` — session goal status, tracking, artifacts, last_sync, recent_events.
...
`.db/system_status.last_sync` MUST be updated to the current timestamp in every post-integration sync (Step 6 of `Scaler-Gateway.md`). A sync that does not update `last_sync` is incomplete.

### P-LAW-007 — scaler_state Updated Before Each Cycle (prevents stale operational state)
`.db/pipeline_scaler_os.yaml` MUST be read and updated at the START of every operation cycle. This includes synchronizing the `state: active_mode` fields (`action_gate`, `input_mode`, `output_level`) to mirror the current `.db/.system.board.yaml` scaler scope values. This prevents acting on stale or desynced parameters.
...
> **Known limitation (audit-acknowledged).** This rule is currently honor-system: no programmatic gate verifies the read. A future enhancement could store per-session runbook acknowledgements (timestamp + content-hash) in `.db/pipeline_scaler_os.yaml.runbook_readiness` and enforce pre-flight refusal on missing/stale acks. Carved out of the 2026-05-18 substrate audit (Cluster D — deferred). Until enforced, agents are bound by P-LAW-008 on trust.
...
This ensures that the global sync workers have updated their respective YAMLs and that `MASTER_INDEX.yaml` and `.db/.system.board.yaml` reflect the absolute truth of the on-disk state. Direct interpreter invocation (`.venv\Scripts\python.exe`) is prohibited per the cross-OS portability law.

**Verification Protocols:**
- **Pre-Flight Check**: Verify `.db/pipeline_scaler_os.yaml` exists and has a valid phase before initiating sync.
- **Post-Flight Check**: Confirm `.db/.system.board.yaml` is dynamically re-assembled and verified by the master sync engine.

### P-LAW-018 — Mode-Specific Configuration Profile
The Scaler MUST load the active configuration profile from `.db/.system.board.yaml` matching the source of the operation (`INTERNAL` or `EXTERNAL`).
...
P-LAW-001 mandates atomic writes; P-LAW-019 mandates the recovery procedure when an atomic write fails. Every Scaler operation that writes to multiple stores in a single transaction (the "atomic trio") — typically `card file` + `[Pillar].sources_ledger.yaml` or `[Pillar].proposals_ledger.yaml` + `.db/pipeline_scaler_os.yaml` — MUST treat the trio as all-or-nothing.
...
3. **Roll back any successful writes from the same operation.** Examples:
   - If the card file was written but `[Pillar].sources_ledger.yaml` failed to update → delete the card file. Revert `.db/pipeline_scaler_os.yaml.gateway_metrics` increments. Revert `scaler_review_queue` posting if any.
   - If both card and ledger succeeded but `.db/pipeline_scaler_os.yaml` failed → delete the card, remove the ledger entry, revert any review-queue posting. The trio is all-or-nothing.
...
4. **Log the failure** to `.db/communication_hubs.scaler_hub.messages` with:
...
- A discovery whose `unread_assets[]` count > 0 MUST be flagged in `.db/pipeline_scaler_os.yaml.state.deferred_assets_count`. The Scaler MAY still draft a Proposal Card from the readable subset, but the card MUST list the deferred assets in `scaler_notes`.
...
2. When executing an OS architectural audit, the scaler operates in INTERNAL mode. The identity documents (`.meta/.os/.system.identity/`) serve as the runbook for structural changes.
...
5. **MANDATORY:** For EXTERNAL operations — consult the relevant `[Pillar].sources_ledger.yaml` (anti-duplication via content hash) BEFORE starting. Items in `.scaler_mixed_inbox/` also gate against `.scaler_mixed_inbox.ledger.yaml`. The auto-sync rolls all per-pillar split files into the central `.db/` rollups.

6. **MANDATORY:** For INTERNAL operations — consult the relevant `[Pillar].proposals_ledger.yaml` under `tracked_gaps` before starting.

7. **MANDATORY:** Update `.db/pipeline_scaler_os.yaml` at the start AND end of every operation cycle.
