...
> **PREVENTION RULE**: Before creating any card, check `.db/pipeline_scaler.board.yaml → gateway_metrics.pending_approvals_count`. If > 0, address or acknowledge pending cards first before creating new ones (unless unrelated scope).
...
1. Read `.db/pipeline_scaler.board.yaml` → confirm current phase and gateway metrics.
...
4. Update `.db/pipeline_scaler.board.yaml → state.current_phase` to the appropriate phase.
...
- Update `.db/pipeline_scaler.board.yaml → gateway_metrics.pending_approvals_count` (increment by 1 if PLANNING mode).
- Update `.db/pipeline_scaler.board.yaml → gateway_metrics.last_gateway_action`.
...
After card + ledger are both written, the Scaler checks the `action_gate` map in `.db/.system.board.yaml` for the card's `integration_type`:
...
- Post a review request in `.db/communication.scaler_review_queue`:
  ```yaml
  - id: [card_id]
    ...
    status: PENDING
  ```
- Update `.db/pipeline_scaler.board.yaml → active_triage[]` with the card.
...
- **`APPROVED`**: Set `user_decision: APPROVED` in the card. Update review queue entry `status: APPROVED`. Atomically update `.db/pipeline_scaler.board.yaml` to decrement `gateway_metrics.pending_approvals_count` by 1 and increment `gateway_metrics.integration_queue_count` by 1. Proceed to Step 5.
...
1. Update `.db/pipeline_scaler.board.yaml`:
...
- [ ] `.db/pipeline_scaler.board.yaml` updated (phase, metrics, gateway_metrics)
...
> **PREVENTION — New Aspect Rule**: If analysis reveals a need for a new aspect/scope, NEVER create it directly. Post in `.db/system_status.scope_suggestions[]` and halt. Await explicit user approval before creating any folder structure for a new aspect. This applies in ALL modes (PLANNING and EXECUTION).
...
4. New cross-toolbox edges are added via UPGRADE cards under the
   `dependency_graph.edges` block of `.db/toolboxes.board.yaml`.
...
| Operation | Card file | `[Pillar].sources_ledger.yaml` | `[Pillar].proposals_ledger.yaml` | `.scaler_mixed_inbox.ledger.yaml` | `.db/pipeline_scaler.board.yaml` | `.db/.system.board.yaml` | `.db/.system.board.yaml` (via sync) | Recovery |
|---|---|---|---|---|---|---|---|---|
...
| Operation | Card file | `[Pillar].proposals_ledger.yaml` | `[Pillar].sources_ledger.yaml` | `.db/pipeline_scaler.board.yaml` | `.db/.system.board.yaml` | `.db/.system.board.yaml` (via sync) | Recovery |
|---|---|---|---|---|---|---|---|
