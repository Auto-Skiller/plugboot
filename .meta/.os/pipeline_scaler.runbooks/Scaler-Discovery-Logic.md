...
4. **Human gate (last resort)** — if specificity AND recency tie AND no pending merge candidate exists, the Scaler MUST stop, post the tie in `.db/.system.board.yaml.communication_hubs.scaler_hub.scaler_review_queue` with `status: TIE_PENDING_RESOLUTION`, and resume only after the user picks a type. Never coin-flip; never default-bias to one type silently.
...
**Update order is mandatory**: pillar `sources_ledger` written first → master rollup at `.db/pipeline_scaler_os.yaml` re-aggregated by sync after.
