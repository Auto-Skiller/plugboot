...
function setupActions() {
    ...
    const btnStop = document.getElementById('btn-stop-dashboard');
    if(btnStop) {
        btnStop.addEventListener('click', async () => {
            if(confirm("Are you sure you want to shut down the Agentic OS Dashboard Server?")) {
                btnStop.innerHTML = '<span class="icon">🛑</span> Shutting down...';
                try {
                    ...
                    setTimeout(() => {
                        document.body.innerHTML = '<div style="display:flex; height:100vh; width:100vw; justify-content:center; align-items:center; background:#0f111a; color:#fff; font-family:sans-serif;"><h1>Dashboard Server Offline</h1></div>';
                    }, 1000);
                } catch(e) {
                    console.error("Stop failed:", e);
                    btnStop.innerHTML = '<span class="icon">🛑</span> Stop';
                }
            }
        });
    }
...
function renderCore(core) {
    ...
    // System Errors
    const errorsContainer = document.getElementById('core-system-errors');
    const errors = core.system?.system_errors || [];
    if(errors.length === 0) {
        errorsContainer.innerHTML = '<div class="empty-state">System Health Normal. No errors.</div>';
    } else {
        errorsContainer.innerHTML = errors.map(e => `<div class="log-item" style="color: #ff6b6b; border-left: 3px solid #ff6b6b; font-weight: bold;">${e}</div>`).join('');
    }
}
