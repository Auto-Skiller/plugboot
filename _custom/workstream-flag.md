...
System now prefers a session-scoped pointer keyed by runtime/session identity
(`SYSTEM_SESSION_KEY`, `CODEX_THREAD_ID`, `CLAUDE_CODE_SSE_PORT`, terminal session IDs,
or the controlling TTY). This keeps concurrent sessions isolated while preserving
compatibility for runtimes that do not expose a stable session key.

## Session Identity Resolution

When System resolves the session-scoped pointer in step 3 above, it uses this order:

1. Explicit runtime/session env vars such as `SYSTEM_SESSION_KEY`, `CODEX_THREAD_ID`,
...
If none of those produce a stable identity, System does not keep probing. It falls
back directly to the shared `.planning/active-workstream` file.

This matters in headless or stripped environments: when stdin is already
non-interactive, System intentionally skips shelling out to `tty` because that path
...
- If that was the last pointer for the repo, System also removes the now-empty
  per-project temp directory
- If sibling session pointers still exist, the temp directory is left in place
- When a pointer refers to a workstream directory that no longer exists, System
...
System does not currently run a background garbage collector for historical temp
...
All workflow routing commands include `${SYSTEM_WS}` which:
...
├── milestones/         # Shared
├── codebase/           # Shared
├── active-workstream   # shared fallback only
└── workstreams/
...
```bash
# All sdk query commands accept --ws
sdk query state.json --ws feature-a
sdk query find-phase 3 --ws feature-b

# Session-local switching without --ws on every command
SYSTEM_SESSION_KEY=my-terminal-a sdk query workstream.set feature-a
SYSTEM_SESSION_KEY=my-terminal-a sdk query state.json
SYSTEM_SESSION_KEY=my-terminal-b sdk query workstream.set feature-b
SYSTEM_SESSION_KEY=my-terminal-b sdk query state.json

# Workstream CRUD
sdk query workstream.create <name>
sdk query workstream.list
sdk query workstream.status <name>
sdk query workstream.complete <name>
```
