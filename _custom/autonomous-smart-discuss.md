...
Smart discuss is the autonomous-optimized variant of `discuss-phase`. It proposes grey area answers in batch tables — the user accepts or overrides per area — then writes an identical CONTEXT.md to what discuss-phase produces.

**Inputs:** `PHASE_NUM` from execute_phase. Run init to get phase paths:

```bash
PHASE_STATE=$(sdk query init.phase-op ${PHASE_NUM})
```
...
```bash
DETAIL=$(sdk query roadmap.get-phase ${PHASE_NUM})
```
...
```bash
sdk query commit "docs(${PADDED_PHASE}): smart discuss context" "${phase_dir}/${padded_phase}-CONTEXT.md"
```
