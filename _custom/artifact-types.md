...
This reference documents all artifact types in the System planning taxonomy. Each type has a defined
...
### Spike README.md / MANIFEST.md (per-spike, via /spike)
- **Shape**: YAML frontmatter (spike, name, validates, verdict, related, tags) + run instructions + results
- **Lifecycle**: Created by `/spike` → Verified → Wrapped up by `/spike-wrap-up`
- **Location**: `.planning/spikes/NNN-name/README.md`, `.planning/spikes/MANIFEST.md`
- **Consumed by**: `/spike-wrap-up` for curation; `pause-work` for spike context handoff
