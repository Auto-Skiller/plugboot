...
| **5. Goal Mgmt** | `task{}` + `context_scan[]` | `goal{}` | Goal exists in `.core/mission_board/` AND (status != done OR is persistent) | If goal is `done ✅` (non-persistent), ⚠️ SOFT PASS (mark task complete, pick next). Else ❌ RETRY creation. |
| **6. Context Deep** | `goal{}` | `context_deep[]` | Goal folder exists in `.core/mission_board/[SESSION_ID]/[GOAL_ID]/` | ✅ PROCEED even if empty |
...
| **10. Sync** | `result{}` | Sync actions | `CONTROLER.yaml` write succeeded. If goal is `persistent ♾️`, do NOT mark done, log cycle. **Update `sessions.active[].last_action`.** | ❌ RETRY. If fail → log to `.core/mission_board/[SESSION]/[GOAL]/scratch.md` |
...
| Scenario | Mode | Action |
|----------|------|--------|
| Active session exists | AUTO | **Auto-resume**: Load session context from `.core/mission_board/`, continue |
| Active session exists | STRICT/COLLAB | **Ask**: "Continue SES-XXX or start new?" |
| No sessions exist | Any | **Start new**: Generate `SES-[NNN]`, register in `CONTROLER.yaml` and `.core/mission_board/` |
...
- **Details & Artifacts** → Must be stored in the specific goal directory: `.core/mission_board/[SESSION_ID]/[GOAL_ID]/`
