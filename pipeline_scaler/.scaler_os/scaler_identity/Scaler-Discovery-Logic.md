...
| **BUILD_NEW** | No matching target exists in the interrogated Pillar. | **Registry Compliance:** Must be registered in Correct Routers. | 1. Define Structure → 2. Create Files → 3. Register in `.meta_os/meta_db/` routers. |
...
**Update order is mandatory**: pillar `sources_ledger` written first → master rollup at `.meta_os/meta_db/pipeline_scaler_os.yaml` re-aggregated by sync after.
