...
| `CARD_INTEGRATED` | Phase 5 finishes all `files_involved` actions | `card_id`, `files_touched[]`, `at` | ledger, `.meta_os/meta_db/pipeline_scaler_os.yaml.metrics` |
...
| Event | Emitted by | Payload | Consumer |
|---|---|---|---|
| `AUDIT_STARTED` | Audit Pass acquires the lock | `triggered_by` (`manual|goal_complete|drift_suspected|quarter_rotation`), `at` | `.meta_os/meta_db/pipeline_scaler_os.yaml.audit_in_progress: true` |
| `AUDIT_CHECK_RAN` | One of the 7 checks completes | `check_id` (1..7), `findings_count`, `severity_breakdown`, `at` | `.meta_os/meta_db/pipeline_scaler_os.yaml.audit_findings[]` |
...
| Event | Emitted by | Payload | Consumer |
|---|---|---|---|
| `STATE_SYNC` | `.meta_os/meta_db/pipeline_scaler_os.yaml` mirrors CONTROLER pre-cycle (P-LAW-007) | `mirrored_fields[]`, `at` | `.meta_os/meta_db/pipeline_scaler_os.yaml` |
| `LEDGER_ROLLUP_REASSEMBLED` | Auto-aggregator rebuilt `.db/` index rollups | `pillars_aggregated[]`, `at` | router |
| `RUNTIME_ROLLUP_REASSEMBLED` | Auto-aggregator rebuilt `.meta_os/meta_db/meta_os.yaml` | `at` | router |
| `RUNBOOK_READINESS_LOGGED` | P-LAW-008 acknowledgement (when enforced) | `runbook_file`, `content_hash`, `at` | `.meta_os/meta_db/pipeline_scaler_os.yaml.runbook_readiness` |
...
2. **No emission to Hustler stores.** Scaler events are written to `.meta_os/meta_db/pipeline_scaler_os.yaml`, `scaler_hub` (inside CONTROLER's communication_hubs), or per-pillar ledgers. Never to `hustler_state.yaml`, `hustler_hub`, or any Hustler ledger.
...
5. **Audit-Pass emissions are observation-only.** Audit events do not cause writes outside `.meta_os/meta_db/pipeline_scaler_os.yaml` and the audit-remediation Mega-YAML — they describe state, never change it.
