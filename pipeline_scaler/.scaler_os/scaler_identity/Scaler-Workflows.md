...
- **Step 7: Aspect & Pending Linking**: Update the existing `[Pillar].sources_ledger.yaml` entry (created during Phase 1) with the newly resolved `primary_aspect` and `aspects[]` list. Multi-pillar items update each respective pillar's entry. The auto-sync re-aggregates the `.meta_os/meta_db/pipeline_scaler_os.yaml` rollup.
...
- **Sync**: Update `CONTROLER.yaml` and all needed scaler related files (e.g. `.meta_os/meta_db/pipeline_scaler_os.yaml`) and trigger `.meta/engine/engines/meta_engine.py` to self-heal the system map.
...
- **Milestones vs. Scaler Ledgers**: The milestones (in `.meta_os/meta_milestones/`) track the High-Level Goal. The `scaler_ledgers/` inside `.scaler_os/scaler_identity/` act as the deep, granular ledgers mapping out those actual files, their current paths, and processing states.
...
- **Inputs**:
  ...
  - For INTERNAL: `.meta_os/meta_identity/`, `CONTROLER.yaml`, `.toolboxes/`, `CONTROLER.yaml`
- **Outputs**:
...
- **Inputs**:
  ...
  - Toolbox catalog `.toolboxes/` and router `.meta_os/meta_db/.toolboxes.yaml`.
...
- **Outputs**:
  ...
  - Post-integration sync: `.meta_os/meta_db/pipeline_scaler_os.yaml`, `CONTROLER.yaml.last_sync` + `recent_events`, `meta_os.yaml` alignment via `.meta/engine/engines/meta_engine.py`.
...
- **Goal-completion trigger**: When a Scaler-related goal is marked `done` in `.meta_os/meta_milestones/`, the Scaler MAY queue an Audit Pass for the next cycle (configurable via `.meta_os/meta_db/pipeline_scaler_os.yaml.audit_policy`).
...
- `.meta_os/meta_db/pipeline_scaler_os.yaml.state.last_audit` + `audit_findings[]`
...
| 5 | **Router freshness** | Compare central ledgers in `.meta_os/meta_db/` against live disk state. Any mismatch indicates a missed `.meta/engine/engines/meta_engine.py` run after a recent integration. |
...
1. **Pre-flight**: Confirm P-LAW-008 runbook immersion is fresh; lock `.meta_os/meta_db/pipeline_scaler_os.yaml.state.audit_in_progress: true` to prevent concurrent cycles from conflicting.
...
3. **Aggregate findings**: append to `.meta_os/meta_db/pipeline_scaler_os.yaml.state.audit_findings[]` with the run timestamp.
4. **Decision**:
...
- The Audit Pass is **bounded in time**: if any check exceeds the `audit_check_timeout` configured in `.meta_os/meta_db/pipeline_scaler_os.yaml` (default 5 minutes per check), it is logged as `INCOMPLETE` and the cycle continues to the next check.
- Findings remain in `.meta_os/meta_db/pipeline_scaler_os.yaml.state.audit_findings[]` for at least 1 quarter (rotates with `.scaler_os/scaler_runtime/.scaler_archive/`) for traceability.
