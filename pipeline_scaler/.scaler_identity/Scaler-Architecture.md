...
- `meta_identity/`: Core identity, routing rules, and execution flow (located in `.identity/`).
...
- `.milestones/`: Active session and goal operation tracking.
- `.db/pipeline_scaler.yaml`: The localized DB configuration file in `.db/`. Acts as the absolute tracking point for the pipeline state.
- `_toolboxes/`: Core agentic and extended capabilities. **Toolboxes must be strictly used via meta routing during every single action in the pipeline execution (e.g., using specific tools for analytics, planning, drafting).**
...
- `.scaler_db/`: Deep, granular tracking ledgers of every file, gap, and proposal processed during pipeline execution.
...
└── .scaler_db/                  ← split sub-ledgers + mixed-inbox ledger
...
The global `_os/engine/meta_sync.py` engine aggregates totals across all per-pillar split files directly into `.db/` rollups. There is no separate pipeline-level routing folder.
...
- **Foundational_Integrity** (Goal: Stability): Consult `CONTROLER.yaml` and the pipelines router at `.db/pipeline_scaler.yaml`. Read relevant laws in `.identity/` and workflow logic in `pipeline_scaler/.scaler_identity/`.
- **Operational_Muscles** (Goal: Power): Consult `.db/.toolboxes.yaml`. Read relevant `yaml_path` entries to see if discovery matches existing descriptions or triggers.
...
|---|---|---|
| `routing_and_syncing` | Master router, sync engine scripts, auto-generated router YAMLs | `CONTROLER.yaml`, `.db/.meta_routing/`, `.db/.meta_routing/meta_sync_engines/` |
| `identity_rules` | OS behavioral laws, modes, decision-making, communication style, personas | `.identity/Modes.md`, `Decision_Making.md`, `Rules_And_Considerations.md`, `Communication_Style.md`, `Persona.md` |
| `identity_architecture` | OS structural docs, naming conventions, architecture diagrams, hierarchy definitions | `.identity/Core_Architecture.md`, `Hierarchy.md`, `Naming-Conventions.md`, `Orchestration_And_Flow.md` |
| `identity_capabilities` | Agent behavioral guides, coding guidelines, skill contracts, Python standards, quick-start refs | `.identity/Core_Capabilities.md`, `Quick_Start.md`, `Universal_Portability_Standard.md`, `Orchestrator_Engine.md` |
| `identity_operational` | Controller guide, session template, pipeline-aware operational guides | `.identity/Controler_Guide.md`, `Session_Template.md` |
| `core_toolbox` | Core cognitive loop toolboxes: analysis, research, planning, brainstorming, benchmarking, documentation, evaluation, notebooklm | `_toolboxes/` |
| `extended_toolbox_business` | Business domain toolboxes (selling, acquisition, monetization tools) | `_toolboxes/` |
| `extended_toolbox_engineering` | Engineering domain toolboxes (coding, devops, automation) | `_toolboxes/` |
| `extended_toolbox_life` | Life domain toolboxes | `_toolboxes/` |
| `extended_toolbox_studio` | Studio/creative domain toolboxes | `_toolboxes/` |
| `mission_board` | Session and goal tracking files, runtime state | `.milestones/` |
...
|---|---|---|---|
| `.scaler_identity/` | **Logic, routing, runbooks** | `SCALER_CONTRACTS.yaml`, `scaler_runbooks/` | Active discoveries, raw external data, scratch drafts, integrated cards (those archive to runtime), user-space content, `.scaler_routing` (deprecated) |
...
### 6.1 Placement Rules
- A new **runbook** belongs in `.scaler_identity/` — never in runtime, never in EXTERNAL_SOURCES.
- A new **scratch draft** during Phase 3 Capability Engineering belongs in `.scaler_runtime/.scaler_scratch/` — never in `.scaler_identity/`, never in a gateway folder.
...
- A new **draft card** during Phase 4 Architecting & Proposing lands in `[Pillar]_external_proposals/` or `[Pillar]_internal_proposals/` — never in `.scaler_identity/`, never in `.scaler_runtime/`.
...
- A new **ledger entry** is appended to the matching `[Pillar].sources_ledger.yaml` or `[Pillar].proposals_ledger.yaml` inside `.scaler_db/` — never to a runtime file, never to an auto-generated rollup directly (the rollup is read-only product of `meta_sync.py`).

### 6.2 Cross-Layer Reads (allowed)
The four zones are write-disjoint but read-permissive:
- `.scaler_identity/` may freely reference any path for documentation purposes.
- The master sync engine `_os/engine/meta_sync.py` reads from all four zones to assemble the master routers.
- Audit Pass (`Scaler-Workflows.md §7`) reads across all four zones — but only writes back to `.db/pipeline_scaler.yaml` and (conditionally) a new INTERNAL Mega-YAML in a gateway folder.

> **Why this matters**: Without explicit "Does NOT contain" rules, agents periodically drop scratch files into `.scaler_identity/`, leak runbook fragments into `.scaler_runtime/`, or draft cards in `.scaler_scratch/`. §6 makes the negative space explicit so placement violations surface during the Audit Pass instead of silently bloating the wrong zone.
...
Components governed by structured schemas and synced via automated python engines (`_os/engine/meta_sync.py`):
- **`pipeline_scaler_state`**: State management housed in `.db/pipeline_scaler.yaml`.
- **`scaler_ledgers`**: Granular per-pillar sub-ledgers in `pipeline_scaler/.scaler_db/`, which are automatically aggregated into central `.db/` rollups.

### 7.2 Cognitive Mapping
Unstructured staging areas and active discovery hubs that MUST NOT be auto-synced by scripts:
- **`external_discoveries`**: `pipeline_scaler/_SCALER-EXTERNAL_SOURCES/[Pillar]_discoveries/`
- **`external_sources_root`**: `pipeline_scaler/_SCALER-EXTERNAL_SOURCES/`
