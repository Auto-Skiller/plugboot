...
- **Atomic State**: Every operation must be preceded by a state update in `.db/pipeline_scaler.yaml`. You MUST explicitly synchronize `.db/pipeline_scaler.yaml` (`active_mode` block) with `CONTROLER.yaml` prior to acting to prevent configuration drift.
...
1. **Pillar Dominance**: The identity context (`_context/.identity/`) rules always take precedence. You can adopt the discovery to become compatible with the current architecture.
2. **Deterministic Precedence**: The Master DB Index (`.db/DB.yaml`) is the final authority. If a proposal contradicts the Index without a plan for a safe migration, it must be rejected. We need an adopting plan for a safe migration.
...
`.db/pipeline_scaler.yaml` MUST be read and updated at the START of every operation cycle. This includes synchronizing the `state: active_mode` fields (`action_gate`, `input_mode`, `output_level`) to mirror the current `CONTROLER.yaml` scaler scope values. This prevents acting on stale or desynced parameters.
...
> **Known limitation (audit-acknowledged).** This rule is currently honor-system: no programmatic gate verifies the read. A future enhancement could store per-session runbook acknowledgements (timestamp + content-hash) in `.db/pipeline_scaler.yaml.runbook_readiness` and enforce pre-flight refusal on missing/stale acks. Carved out of the 2026-05-18 substrate audit (Cluster D — deferred). Until enforced, agents are bound by P-LAW-008 on trust.
...
- **Pre-Flight Check**: Verify `.db/pipeline_scaler.yaml` exists and has a valid phase before initiating sync.
- **Post-Flight Check**: Confirm `.db/MASTER_INDEX.yaml` is dynamically re-assembled and verified by the master sync engine.
...
P-LAW-001 mandates atomic writes; P-LAW-019 mandates the recovery procedure when an atomic write fails. Every Scaler operation that writes to multiple stores in a single transaction (the "atomic trio") — typically `card file` + `[Pillar].sources_ledger.yaml` or `[Pillar].proposals_ledger.yaml` + `.db/pipeline_scaler.yaml` — MUST treat the trio as all-or-nothing.
...
3. **Roll back any successful writes from the same operation.** Examples:
   - If the card file was written but `[Pillar].sources_ledger.yaml` failed to update → delete the card file. Revert `.db/pipeline_scaler.yaml.gateway_metrics` increments. Revert `scaler_review_queue` posting if any.
   - If both card and ledger succeeded but `.db/pipeline_scaler.yaml` failed → delete the card, remove the ledger entry, revert any review-queue posting. The trio is all-or-nothing.
...
- A discovery whose `unread_assets[]` count > 0 MUST be flagged in `.db/pipeline_scaler.yaml.state.deferred_assets_count`. The Scaler MAY still draft a Proposal Card from the readable subset, but the card MUST list the deferred assets in `scaler_notes`.
...
2. When executing an OS architectural audit, the scaler operates in INTERNAL mode. The identity documents (`_context/.identity/`) serve as the runbook for structural changes.
...
7. **MANDATORY:** Update `.db/pipeline_scaler.yaml` at the start AND end of every operation cycle.
