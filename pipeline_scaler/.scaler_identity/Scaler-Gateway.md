...
> **PREVENTION RULE**: Before creating any card, check `.db/pipeline_scaler.yaml → gateway_metrics.pending_approvals_count`. If > 0, address or acknowledge pending cards first before creating new ones (unless unrelated scope).
...
1. Read `.db/pipeline_scaler.yaml` → confirm current phase and gateway metrics.
...
4. Update `.db/pipeline_scaler.yaml → state.current_phase` to the appropriate phase.
...
- Update `.db/pipeline_scaler.yaml → gateway_metrics.pending_approvals_count` (increment by 1 if PLANNING mode).
- Update `.db/pipeline_scaler.yaml → gateway_metrics.last_gateway_action`.
...
- Post a review request in `CONTROLER.yaml → communication.scaler_review_queue`:
  ...
  - id: [card_id]
    ...
    status: PENDING
  ```
- Update `.db/pipeline_scaler.yaml → active_triage[]` with the card.
...
1. Update `.db/pipeline_scaler.yaml`:
...
- [ ] `.db/pipeline_scaler.yaml` updated (phase, metrics, gateway_metrics)
...
> This protocol closes the loop between Scaler discovery and `_toolboxes/` population.
...
integration_strategy:
  ...
  toolbox_target:
    toolbox_path: "_toolboxes/engineering/coding"
    toolbox_yaml: "_toolboxes/engineering/coding/coding.yaml"
...
4. New cross-toolbox edges are added via UPGRADE cards under the
   `dependency_graph.edges` block of `.db/.toolboxes.yaml`.
...
| Operation | Card file | `[Pillar].sources_ledger.yaml` | `[Pillar].proposals_ledger.yaml` | `.scaler_mixed_inbox.ledger.yaml` | `.db/pipeline_scaler.yaml` | `CONTROLER.yaml` | `MASTER_INDEX.yaml` (via sync) | Recovery |
|---|---|---|---|---|---|---|---|---|
...
| Operation | Card file | `[Pillar].proposals_ledger.yaml` | `[Pillar].sources_ledger.yaml` | `.db/pipeline_scaler.yaml` | `CONTROLER.yaml` | `MASTER_INDEX.yaml` (via sync) | Recovery |
|---|---|---|---|---|---|---|---|
...
| Operation | `.db/pipeline_scaler.yaml` | `CONTROLER.yaml` | `MASTER_INDEX.yaml` (via `_os/engine/meta_sync.py`) | `.db/.toolboxes.yaml` (auto) | `.db/.core.yaml` (auto) | Recovery |
|---|---|---|---|---|---|---|
